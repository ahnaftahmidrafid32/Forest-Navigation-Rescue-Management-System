...
public class Main {
    public static void main(String[] args) {
        ...
        try {
            storage.ensureReady();
            map = storage.loadMap();
            if (map.getAllNodes().isEmpty() || countEdges(map) == 0) {
                map = buildMap();
                storage.saveMap(map);
            }
        } catch (IOException e) {
    ...
    private static List<Survivor> safeLoadSurvivors(FileStorage storage, ForestMap1 map) {
        try {
            return storage.loadSurvivors(map);
        } catch (IOException e) {
    ...
    private static List<RescueTeam> safeLoadTeams(FileStorage storage, ForestMap1 map) {
        try {
            return storage.loadRescueTeams(map);
        } catch (IOException e) {
            return new ArrayList<>();
        }
    }
    ...
    private static List<Hazard> safeLoadHazards(FileStorage storage, ForestMap1 map) {
        try {
            return storage.loadHazards(map);
        } catch (IOException e) {
            return new ArrayList<>();
        }
    }
